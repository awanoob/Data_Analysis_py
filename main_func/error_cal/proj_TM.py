...
def _proj_TM(bl):
    ...
    longitude = bl[1] * DEG2RAD
    latitude = bl[0] * DEG2RAD
    ...
    sn = a / math.sqrt(1.e0 - TranMerc_es * pow(math.sin(latitude), 2.0))

    tmd = TranMerc_ap * latitude - TranMerc_bp * math.sin(2.e0 * latitude) + TranMerc_cp * math.sin(4.e0 * latitude) - TranMerc_dp * math.sin(6.e0 * latitude) + TranMerc_ep * math.sin(8.e0 * latitude)
    tmdo = TranMerc_ap * b0 - TranMerc_bp * math.sin(2.e0 * b0) + TranMerc_cp * math.sin(4.e0 * b0) - TranMerc_dp * math.sin(6.e0 * b0) + TranMerc_ep * math.sin(8.e0 * b0)
    ...
    # 北向坐标
    xy[0] = fn + t1 + pow(dlam, 2.e0) * t2 + pow(dlam, 4.e0) * t3 + pow(dlam, 6.e0) * t4 + pow(dlam, 8.e0) * t5
    ...
    # 东向坐标
    xy[1] = fe + dlam * t6 + pow(dlam, 3.e0) * t7 + pow(dlam, 5.e0) * t8 + pow(dlam, 7.e0) * t9
